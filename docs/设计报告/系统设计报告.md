...
![discussion_dataflow](/Users/zhengguangyi/Desktop/数据库/大作业/discussion_dataflow.png)
...
7. 讨论帖
   ...
   discussions({<u>id</u>，发布者，发布者头像，发布时间，更新时间，帖子内容，帖子标题，帖子标签，订阅者，喜爱者}，D，DOM，F)

   D = {STR, N，T，J}

   DOM = {DOM(id) = DOM(发布者) = N， DOM(发布者头像) = DOM(内容) = DOM(标题) = DOM(标签) = STR，DOM(订阅者) = DOM(喜爱者) = J，DOM(发布时间) = DOM(更新时间) = T}
