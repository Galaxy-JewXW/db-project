<template>
  <div class="scroll-container">
    ...
    <v-card class="mx-auto main-card" max-width="85%">
      ...
      <!-- 评论区 -->
      <v-divider></v-divider>
      <div v-for="comment in mainDiscussion.comments" :key="comment.id">
        <v-card class="mx-auto comment-card" max-width="85%" style="margin-right: auto; margin-left: auto;">
          ...
          <v-row no-gutters>
            <v-col cols="auto">
              <v-btn
                ...
                class="like-btn"
                :variant="comment.isLiked ? 'tonal' : 'text'"
                :color="'#ee3f4d'"
              >
...
export default {
  ...
  methods: {
    ...
    getCommentPublisherById(id) {
      ...
      const comment = this.mainDiscussion.comments.find((c) => c.id === id);
      return comment ? comment.publisher : "未知用户";
    },
  },
};
</script>
